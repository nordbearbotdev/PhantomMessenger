...
class Dialog(wx.Dialog):
    ...
    def __init__(self, main_window):
        wx.Dialog.__init__(self, main_window, wx.ID_ANY,
                           lang.DSET_TITLE)
        self.mw = main_window
        ...
        outer_sizer.Add(self.notebook, 1, wx.EXPAND | wx.LEFT | wx.TOP
                        | wx.RIGHT, border=5)

        # 1.2 the button_sizer at the bottom

        button_sizer = wx.BoxSizer(wx.HORIZONTAL)
        outer_sizer.Add(button_sizer, 0, wx.ALIGN_RIGHT | wx.ALL,
                        border=5)

        # 1.2.1 cancel button

        btn_cancel = wx.Button(self.outer_panel, wx.ID_CANCEL,
                               lang.BTN_CANCEL)
        ...
        dlg.Text(self.p1, lang.DSET_NET_TOR_ADDRESS, ('tor_portable',
                 'tor_server'), True)
        dlg.Text(self.p1, lang.DSET_NET_TOR_SOCKS, ('tor_portable',
                 'tor_server_socks_port'))
        dlg.Text(self.p1, lang.DSET_NET_TOR_CONTROL, ('tor_portable',
                 'tor_server_control_port'))
        self.s_tor = dlg.Separator(self.p1, 'Tor')
        dlg.Text(self.p1, lang.DSET_NET_TOR_ADDRESS, ('tor',
                 'tor_server'), True)
        dlg.Text(self.p1, lang.DSET_NET_TOR_SOCKS, ('tor',
                 'tor_server_socks_port'))
        dlg.Text(self.p1, lang.DSET_NET_TOR_CONTROL, ('tor',
                 'tor_server_control_port'))
        dlg.Text(self.p1, lang.DSET_NET_OWN_HOSTNAME, ('client',
                 'own_hostname'), True)
        dlg.Separator(self.p1, 'Client')
        dlg.Text(self.p1, lang.DSET_NET_LISTEN_INTERFACE, ('client',
                 'listen_interface'), True)
        dlg.Text(self.p1, lang.DSET_NET_LISTEN_PORT, ('client',
                 'listen_port'))
        self.p1.fit()
        ...
        self.lang = dlg.Text(self.p2, lang.DSET_GUI_LANGUAGE, ('gui',
                             'language'))
        self.lang_old = self.lang.getValue()
        dlg.Check(self.p2, lang.DSET_GUI_OPEN_MAIN_HIDDEN, ('gui',
                  'open_main_window_hidden'))
        dlg.Check(self.p2, lang.DSET_GUI_OPEN_CHAT_HIDDEN, ('gui',
                  'open_chat_window_hidden'))
        dlg.Check(self.p2, lang.DSET_GUI_NOTIFICATION_POPUP, ('gui',
                  'notification_popup'))
        dlg.Check(self.p2, lang.DSET_GUI_FLASH_WINDOW, ('gui',
                  'notification_flash_window'))
